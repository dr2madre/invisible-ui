...
const setup = (props: Record<string, unknown> = {}) =>
  render(SheetDialog, {
    props: { title: "Filters", description: "Refine the results.", ...props },
    slots: {
      trigger: () => "Open panel",
      default: () => [h("label", ["Query ", h("input", { type: "text" })])],
    },
  });

// Native <dialog>: backdrop presses target the element itself with coordinates
// outside its box.
 ...
 * a reliable PointerEvent constructor, so this rides MouseEvent under the
 * pointer type names.
 */
...
describe("Vue SheetDialog", () => {
  ...
  it("renders a grab handle only when draggable, and never on top", async () => {
    const user = userEvent.setup();
    const { unmount } = setup();
    await user.click(screen.getByRole("button", { name: "Open panel" }));
  ...
  it("tracks a bottom drag on the panel transform", async () => {
    const user = userEvent.setup();
    setup({ side: "bottom", draggable: true });
    await user.click(screen.getByRole("button", { name: "Open panel" }));
  ...
  it("dismisses a bottom drag released past the distance threshold", async () => {
    ...
    // No move, so velocity stays 0; only the release distance decides
    // (250 > 25% of 400).
    await fireEvent(handleOf(panel), pointer("pointerdown", { y: 100 }));
  ...
  it("snaps back when released below the threshold", async () => {
    ...
    await fireEvent(handleOf(panel), pointer("pointerdown", { y: 100 }));
    await fireEvent(window, pointer("pointerup", { y: 150 })); // 50 < 25% of 400, velocity 0
  ...
  it("dismisses a lateral drag toward the anchored edge", async () => {
    const user = userEvent.setup();
    setup({ side: "right", draggable: true });
    await user.click(screen.getByRole("button", { name: "Open panel" }));
    const panel = screen.getByRole("dialog");
  ...
  it("ignores a lateral drag away from the anchored edge", async () => {
    ...
    // Dragging leftward (inward) leaves a right-anchored panel open.
    await fireEvent(handleOf(panel), pointer("pointerdown", { x: 500 }));
