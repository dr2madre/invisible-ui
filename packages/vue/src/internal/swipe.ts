...
export interface SwipeDismissHandle {
  /** Replace the options (the region calls this when its props change). */
 ...
 * (a notification), ported from the Svelte adapter's action. Pointer-only by
 * nature, so it is an *enhancement*: the close button and auto-dismiss remain
 * the accessible paths. A press only becomes a swipe once it moves past
 * `activation` px and is more horizontal than vertical (so taps still click
 * and vertical scroll still scrolls). On release, a drag past the distance
 * threshold or a fast flick animates the element off-screen and calls
 * `onDismiss`; anything less snaps back.
 *
 * The helper drives inline `transform`/`opacity` and a `data-swiping` /
 ...
 * is preserved. The caller attaches it in an element ref callback and calls
 * `destroy()` when the element leaves.
 */
