...
describe("Svelte dialog family header", () => {
  it("keeps the close button on Dialog by default and drops it on request", async () => {
    ...
    await rerender({ title: "Settings", open: true, closeButton: false });
    expect(screen.queryByRole("button", { name: "Close" })).toBeNull();
    expect(screen.getByRole("heading", { name: "Settings" })).toHaveClass("dialog-header__title");
  });

  it("shows the SheetDialog description as the subtitle inside the header", () => {
    render(SheetDialog, {
