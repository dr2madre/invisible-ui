        ...
        <button class="stepper__trigger" use:stepAction={index}>
          ...
          <span class="stepper__text">
            ...
            {#if status === "complete"}
              ...
              <span class="stepper__status">{$t("stepper.completed")}</span>
            {/if}
            {#if step.description}
...
<style>
  .stepper {
    /* The query container for the narrow presentation below. Size
       containment removes the intrinsic inline size, so the explicit 100%
       keeps the nav from collapsing inside flex or grid parents. */
  ...
  .stepper__step {
    /* Anchors the hidden status text: unanchored, it would sit at its
       static position outside any clipping and widen the page. */
    position: relative;
    display: flex;
    flex: 1;
    align-items: center;
  ...
  .stepper__connector {
    ...
    background-image: radial-gradient(
      circle,
      var(--ds-step-connector-color, var(--ds-color-border, #c7c1b7)) 0 42%,
      transparent 48%
    );
  ...
  .stepper__step[data-status="current"] .stepper__connector {
    background-image: radial-gradient(
      circle,
      var(--ds-step-connector-active, var(--ds-color-secondary, #7a52cc)) 0 42%,
      transparent 48%
    );
  }
  ...
  /* Too narrow for one row: the horizontal presentation becomes the vertical
     one. Same geometry as the rules above, so a connector never stays
     horizontal pointing at empty space. The DOM order, the focus order and the
     public `orientation` prop are untouched. */
  ...
  .stepper__indicator {
    ...
    line-height: 1;
    border-radius: 50%;
    border: 2px solid var(--ds-step-indicator-border, var(--ds-color-border, #c7c1b7));
    color: var(--ds-step-indicator-text, var(--ds-color-text-secondary, #524c44));
    background: var(--ds-step-indicator-bg, var(--ds-color-surface, #e6e0d8));
    transition:
  ...
  .stepper__step[data-status="complete"] .stepper__indicator {
    background: var(--ds-step-complete-bg, var(--ds-color-surface, #e6e0d8));
    border-color: var(--ds-step-complete-border, var(--ds-color-border, #c7c1b7));
    color: var(--ds-step-complete-text, var(--ds-color-text, #282420));
  }
  ...
  .stepper__step[data-status="current"] .stepper__indicator {
    background: var(
      --ds-step-current-bg,
      color-mix(in srgb, var(--ds-color-selected, #7a52cc) 10%, transparent)
    );
    border-color: color-mix(in srgb, var(--ds-color-selected, #7a52cc) 35%, transparent);
    color: var(--ds-step-current-text, var(--ds-color-selected, #7a52cc));
  }
  /* Upcoming: no fill, dashed near-black ring. */
  .stepper__step[data-status="upcoming"] .stepper__indicator {
    background: transparent;
    border-style: dashed;
    border-color: var(--ds-step-upcoming-border, var(--ds-color-text, #282420));
    color: var(--ds-step-upcoming-text, var(--ds-color-text, #282420));
  }
  ...
  /* Read by screen readers, shown by the checkmark instead. */
  .stepper__status {
    position: absolute;
    inline-size: 1px;
    block-size: 1px;
  ...
  /* A long or translated label wraps onto more lines. It is never truncated
