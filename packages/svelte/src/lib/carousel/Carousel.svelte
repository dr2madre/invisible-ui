...
>
  ...
  {#if showIndicators}
    <div class="carousel__indicators" role="group" aria-label={$t("carousel.choose")}>
...
<style>
  ...
  /* The dot the user sees is drawn inside the button, so the button itself
     can be a 24px square target without growing the dot. */
  .carousel__dot {
    display: inline-flex;
    align-items: center;
    justify-content: center;
