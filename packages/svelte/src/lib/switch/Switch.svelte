...
<style>
  ...
  .switch {
    ...
    /* Off, the track is a plain surface with a boundary drawn round it. The
       surface alone is too close to the page to be seen, the boundary is what
       makes the control visible. It is drawn inside, so nothing shifts. */
  ...
  .switch__input:checked + .switch {
    ...
    /* On, the fill itself is far enough from the page: no boundary needed. */
    box-shadow: none;
  }
