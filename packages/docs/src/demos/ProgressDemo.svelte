<script>
  ...
  // Determinate bar: refills forever so the "completes" behaviour stays visible.
  let progress = 0;
  onMount(() => {
    const t = setInterval(() => {
...
<div class="demo">
  <section>
    <p class="demo__caption">Bar and circle</p>
    <div style="max-inline-size: 22rem; display: flex; flex-direction: column; gap: 1rem;">
      <Progress value={62} label="Uploading" />
      <Progress value={null} label="Working" />
      <div style="display: flex; gap: 1rem; align-items: center;">
        <Progress shape="circle" value={68} showValue label="Exporting" />
        <Progress shape="circle" value={25} label="Uploading photo" />
        <Progress shape="circle" value={null} label="Working" />
      </div>
    </div>
  </section>
