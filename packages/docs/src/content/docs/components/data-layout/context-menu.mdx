...
<ImportSnippet component="context-menu" />

In Elements, the children are the region. Assign the actions through the
`items` property and listen for `select`, which fires after the menu closes:
