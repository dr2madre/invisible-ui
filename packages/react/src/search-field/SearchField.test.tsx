...
describe("React SearchField", () => {
  it("renders a named search input and native submit action", () => {
    render(<SearchField label="Search tables" />);
  ...
  it("drops the submit button for a filter that applies as you type", () => {
    const { container } = render(<SearchField label="Search tables" submitButton={false} />);
  ...
  it("clears once and returns focus to the input", async () => {
    ...
    render(<SearchField label="Search tables" value="archive" onValueChange={onValueChange} />);
  ...
  it("has no accessibility violations with its label hidden", async () => {
    const { container } = render(<SearchField label="Search tables" hideLabel value="archive" />);
