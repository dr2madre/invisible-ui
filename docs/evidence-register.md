...
| The declarations the published packages ship type-check for a consumer | `CONTRIBUTING.md`, "Running the checks locally" | gate: `scripts/check-packed-consumers.mjs` compiles a throwaway consumer against the packed tarballs; 7 tests in `scripts/consumer-declarations.test.mjs` hold the verdict to the diagnostics in `@design-system/*` and in the consumer file | held for what this repository publishes. A dependency's own broken declarations do not fail the check, by design: no consumer compiles them with `skipLibCheck` off |
